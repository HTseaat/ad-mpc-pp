...
for ((layer_number = 1; layer_number <= layers; layer_number++)); do
    ...
    printf '[node%s_cont%s] LAUNCHED global_id=%s port=%s\n' \
        "$node_number" "$layer_number" "$file_number" "$external_port"
    (
        set -o pipefail
        MPC_IMAGE="$MPC_IMAGE" timeout --signal=TERM --kill-after=30s "${run_timeout}s" \
            ...
            -e "ZMQ_AUTH_MODE=${ZMQ_AUTH_MODE:-curve}" \
            -e "ZMQ_CURVE_READY_TIMEOUT=${ZMQ_CURVE_READY_TIMEOUT:-180}" \
            -e "SHUFFLE_K=${SHUFFLE_K:-128}" \
            -e "SHUFFLE_MODE=${SHUFFLE_MODE:-iterated}" \
            -e "SHUFFLE_ACK_TIMEOUT=${SHUFFLE_ACK_TIMEOUT:-900}" \
            -e "SHUFFLE_ACK_THRESHOLD=${SHUFFLE_ACK_THRESHOLD:-$node_count}" \
            -e "ADMPC_SHUFFLE_RUN_ID=${ADMPC_SHUFFLE_RUN_ID:-$conf_dir}" \
            htadkg_adkg /opt/venv/admpc/bin/python3 -u \
            -m scripts.admpc_dynamic_shuffle_run -d \
            -f "conf/${conf_dir}/local.${file_number}.json" -time "$start_time" \
            2>&1 | tee "$log_path" | \
            awk -v prefix="[node${node_number}_cont${layer_number}] " '
                /CURVE channel ready/ {
                    print prefix "CURVE_READY"
                    fflush()
                    next
                }
                /ADMPC_SHUFFLE_FINISHED/ {
                    layer = $0
                    elapsed = $0
                    sub(/^.*"layer": /, "", layer)
                    sub(/,.*$/, "", layer)
                    sub(/^.*"elapsed": /, "", elapsed)
                    sub(/,.*$/, "", elapsed)
                    print prefix "FINISHED layer=" layer " elapsed=" elapsed "s"
                    fflush()
                    next
                }
                /ADMPC_SHUFFLE_RESULT|Traceback|ERROR|Exception|[Tt]imeout/ {
                    print prefix $0
                    fflush()
                }
            '
