...
# Send one archive per node, with bounded parallelism.
deploy_node() {
    i="$1"
    ...
    scp "${SSH_OPTIONS[@]}" "$archive_name" "$ssh_user_host:${REMOTE_ROOT}/admpc/conf"
    ssh "${SSH_OPTIONS[@]}" "$ssh_user_host" -- "cd ${REMOTE_ROOT}/admpc/conf && tar Jxf $archive_name"
