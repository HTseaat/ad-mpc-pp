...
from adkg.bundle import Bundle
...
async def prog(ctx):
    ...
    # 这里异步延迟的设置可能还没有考虑
    dkg = Bundle(public_keys, private_key, g, h, n, t, deg, myid, ctx.send, ctx.recv, pc, curve_params, mat)
    dkg_list[myid] = dkg
    dkg_tasks[myid] = asyncio.create_task(dkg.run_bundle(w, rounds))
