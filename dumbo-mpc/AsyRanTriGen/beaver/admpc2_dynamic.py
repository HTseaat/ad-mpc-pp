...
from beaver.adversarial_faults import BYZANTINE_DELTA, ContinuumFaultController
...
class ADMPC_Dynamic(ADMPC):
    def __init__(self, public_keys, private_key, pkbls, skbls, n, t, srs, my_id, send, recv, total_cm, layers, next_pks = None, layerID = None, admpc_control_instance=None, election_config=None):
        ...
        self.fault_controller = ContinuumFaultController.from_env(
            n=n,
            t=t,
            layers=layers,
        ...
        from beaver.fault_accumulation import FaultAccumulationController
        ...
        self.election_pipeline = None
        if election_config is not None:
            from committee_election.continuum import ContinuumElectionPipeline

            self.election_pipeline = ContinuumElectionPipeline(
                mode=election_config["mode"],
                n=n,
                t=t,
                layers=layers,
                physical_layer_id=layerID,
                ...
                public_key=pkbls,
                private_key=skbls,
                all_public_keys=election_config["all_public_keys"],
                registry=election_config["registry"],
                run_id=election_config["run_id"],
                get_send=self.get_send,
                subscribe_recv=self.subscribe_recv,
            )

    async def before_handoff_election(self):
        if self.election_pipeline is not None:
            await self.election_pipeline.before_handoff()

    async def finalize_elections(self):
        if self.election_pipeline is not None:
            await self.election_pipeline.finalize()

    def prepare_aggtrans_input(self, values):
        """Return the opt-in outgoing AggTrans view without mutating MPC state."""
        attack_index = self.fault_controller.begin_aggtrans_fork(
            len(values.get("proof", []))
        )
        ...
        self.fault_controller.record_aggtrans_fork(
            honest_commitments=values["commitment"],
            forked_commitments=forked["commitment"],
        )
        return forked

    def prepare_batchmul_inputs(self, left_inputs, right_inputs):
        """Return opt-in forked BatchMul input views without mutating state."""
        left_proofs = left_inputs.get("proof", [])
        right_proofs = right_inputs.get("proof", [])
        attack_index = self.fault_controller.begin_batchmul_fork(
            len(left_proofs), len(right_proofs)
        )
        if attack_index is None:
        ...
        forked_left, forked_right = build_batchmul_input_commitment_fork(
            kzg_lib=lib,
            srs_pk=self.srs["Pk"],
            left_payload=left_inputs,
            right_payload=right_inputs,
            n=self.n,
            dealer_local_id=self.my_id,
            attack_index=attack_index,
            delta=BYZANTINE_DELTA,
        )
        self.fault_controller.record_batchmul_fork(
            honest_left_commitments=left_inputs["commitment"],
            forked_left_commitments=forked_left["commitment"],
            honest_right_commitments=right_inputs["commitment"],
            forked_right_commitments=forked_right["commitment"],
        )
    ...
    async def run_admpc(self, start_time):
        ...
        if self.fault_accumulation.continuum_should_be_silent():
            computation_epoch = self.layer_ID
            self.fault_accumulation.log_silent_entered(epoch=computation_epoch)
            logging.warning(
            ...
            return
        if self.election_pipeline is not None:
            await self.election_pipeline.start()
        ...
        if self.layer_ID == 0:
            ...
            await self.fault_controller.delay_handoff_if_needed()
            await self.before_handoff_election()

            aggtrans_inputs = self.prepare_aggtrans_input(add_outputs)
            ...
            trans_pre = Transfer_Pre(self.next_pks, self.private_key, 
                                    ...
                                    transsend, transrecv, cm, aggtrans_inputs, mpc_instance=self)
            ...
            batchmul_left_inputs, batchmul_right_inputs = (
                self.prepare_batchmul_inputs(mul_left_inputs, mul_right_inputs)
            )
            ...
            mul_pre = BatchMul_Pre(self.next_pks, self.private_key, 
                                    self.pkbls, self.skbls, self.n, self.t, self.srs, self.my_id, 
                                    mulsend, mulrecv, cm, batchmul_left_inputs,
                                    batchmul_right_inputs, mpc_instance=self)
            ...
            if self.next_pks is None:
                ...
                batchmul_left_inputs, batchmul_right_inputs = (
                    self.prepare_batchmul_inputs(new_mul_shares, new_mul_shares)
                )
                mul_pre_time = time.time()
                multag = ADMPCMsgType.MUL + str(self.layer_ID+1)
                mulsend, mulrecv = self.get_send(multag), self.subscribe_recv(multag)
