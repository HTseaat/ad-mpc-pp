...
class ADMPC_Dynamic(_NonlinearADMPCDynamic):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._retained_bgw_reductions = []

    def cleanup_bgw_responders(self):
        """Stop BGW responders after the measured protocol has completed."""
        for bgw in self._retained_bgw_reductions:
            bgw.kill()
        self._retained_bgw_reductions.clear()

    def __exit__(self, exc_type, exc_value, traceback):
        # Fallback for exceptions before the runner reaches its post-exec
        # cleanup point.  Normal cleanup happens after LOCAL_METRICS_BARRIER,
        # which is outside exec_time/layer_time.
        self.cleanup_bgw_responders()
    ...
    async def _run_bgw_reduction(self, left_inputs, right_inputs, cm):
        ...
        try:
            ...
            bgw.kill()
            raise
        # run_multiply returns at the protocol's n-t quorum.  Retain its AVID
        # responders so slower honest peers can still retrieve data, without
        # putting responder draining or an all-node barrier in measured BGW
        # latency.  The runner releases them after its post-exec barrier.
        self._retained_bgw_reductions.append(bgw)
