...
class ADMPC_Dynamic(ADMPC):
    ...
    def _register_transfer_batch(self, tag, batch_size, role):
        if self.metrics_recorder is None or not self.metrics_recorder.communication_metrics.enabled:
            return
        batch_size = int(batch_size)
        target_layer = int(tag[len(ADMPCMsgType.TRANS):])
        self.metrics_recorder.register_protocol_batch(
            protocol="aggtrans", tag=tag, source_layer=target_layer - 1,
            target_layer=target_layer, batch_size=batch_size, role=role,
            operation_id=f"aggtrans:{target_layer}", unit="sharing",
        )
    ...
    async def run_admpc(self, start_time):
        ...
        # w indicates the circuit width of each layer.  Keep the paper default
        # at B=100, while making the distributed runner explicit about which
        # width it is measuring.
        w = int(os.environ.get("CIRCUIT_WIDTH", "100"))
        if w <= 0:
            raise ValueError("CIRCUIT_WIDTH must be a positive integer")
        ...
        if self.layer_ID == 0:
            ...
            total_inputs = 2 * w
            inputs_num = total_inputs // self.n + (
                1 if self.my_id < total_inputs % self.n else 0
            )
            ...
            for dealer_id in range(self.n): 
                total_inputs = 2 * w
                inputs_num = total_inputs // self.n + (
                    1 if dealer_id < total_inputs % self.n else 0
                )
            ...
            self._register_transfer_batch(transtag, len(add_outputs['proof']), "source")
            ...
            if self.next_pks is None:
                ...
                self._register_transfer_batch(transtag, len(trans_shares['proof']), "destination")
