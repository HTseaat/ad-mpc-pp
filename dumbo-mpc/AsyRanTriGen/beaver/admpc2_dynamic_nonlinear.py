...
class ADMPC_Dynamic(ADMPC):
    def __init__(self, public_keys, private_key, pkbls, skbls, n, t, srs, my_id, send, recv, total_cm, layers, next_pks = None, layerID = None, admpc_control_instance=None, metrics_recorder=None):
    ...
    def _register_batchmul_batch(self, tag, batch_size, role):
        if self.metrics_recorder is None or not self.metrics_recorder.communication_metrics.enabled:
            return
        batch_size = int(batch_size)
        target_layer = int(tag[len(ADMPCMsgType.MUL):])
        self.metrics_recorder.register_protocol_batch(
            protocol="batchmul", tag=tag, source_layer=target_layer - 1,
            target_layer=target_layer, batch_size=batch_size, role=role,
            operation_id=f"batchmul:{target_layer}", unit="gate",
        )
    ...
    async def run_admpc(self, start_time):
        ...
        if self.layer_ID == 0:
            ...
            base_inputs, extra_inputs = divmod(2 * w, self.n)
            inputs_num = base_inputs + int(self.my_id < extra_inputs)
            ...
            for dealer_id in range(self.n): 
                inputs_num = base_inputs + int(dealer_id < extra_inputs)
            ...
            expected_inputs = 2 * w
            if len(merged_shares) != expected_inputs:
                raise ValueError(
                    f"layer {self.layer_ID}: expected {expected_inputs} input "
                    f"shares for BatchMul width {w}, got {len(merged_shares)}"
                )
            if len(merged_commits) != expected_inputs:
                raise ValueError(
                    f"layer {self.layer_ID}: expected {expected_inputs} input "
                    f"commitments for BatchMul width {w}, got {len(merged_commits)}"
                )
            ...
            if len(left_proof_inputs) != w or len(right_proof_inputs) != w:
                raise ValueError(
                    f"layer {self.layer_ID}: expected BatchMul proof widths "
                    f"{w}/{w}, got {len(left_proof_inputs)}/{len(right_proof_inputs)}"
                )
            if len(left_commit_inputs) != w or len(right_commit_inputs) != w:
                raise ValueError(
                    f"layer {self.layer_ID}: expected BatchMul commitment widths "
                    f"{w}/{w}, got {len(left_commit_inputs)}/{len(right_commit_inputs)}"
                )
            ...
            self._register_batchmul_batch(multag, len(mul_left_inputs['proof']), "source")
            ...
            if self.next_pks is None:
                ...
                if len(new_mul_shares['proof']) != w:
                    raise ValueError(
                        f"layer {self.layer_ID}: expected BatchMul output width "
                        f"{w}, got {len(new_mul_shares['proof'])}"
                    )
                self._register_batchmul_batch(multag, len(new_mul_shares['proof']), "destination")
                ...
                mul_pre_time = time.time()
                multag = ADMPCMsgType.MUL + str(self.layer_ID+1)
                self._register_batchmul_batch(multag, len(new_mul_shares['proof']), "source")
                mulsend, mulrecv = self.get_send(multag), self.subscribe_recv(multag)
                mul_pre = BatchMul_Pre(self.next_pks, self.private_key, 
                                        self.pkbls, self.skbls, self.n, self.t, self.srs, self.my_id, 
