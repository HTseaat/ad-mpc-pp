...
import json
import sys
from beaver.protocol_metrics import proof_metadata, timed_call
import struct
...
class BatchMul_Pre(BatchMultiplication):
    ...
    async def run_multiply(self):
        ...
        proof_metric = proof_metadata(
            self.mpc_instance,
            protocol="batchmul",
            operation="ipakzg_generate",
            dealer_local_id=self.my_id,
            receiver_local_id=None,
            batch_size=len(self.comandproof_left_inputs["proof"]),
        )

        # Generate the multiplication witnesses consumed by both proof systems.
        result = timed_call(
            self.mpc_instance, proof_metric, "multiplication_witness_preparation",
            lib.pyMultiplyClaimedValuesWithAux,
        ...
        # Generate the factor/inner-product portion of IPAKZG.P.
        proof_ptr = timed_call(
            self.mpc_instance, proof_metric, "factor_proof",
            lib_bulletproof.pyProveFactors, input_json,
        )
...
class BatchMul_Foll(BatchMultiplication ):
    ...
    async def acss_step(
            self, msgmode, outputs, mul_num, quorum_proposal, dealer_ready):
            ...
            if majority_oc is not None:
                ...
                if (left_commitments, right_commitments) != majority_oc:
                    ...
                    if fault_controller is not None:
                        fault_controller.record_batchmul_commitment_exclusion(
                ...
                for oc_val, count in commit_counter.items():
                    if count >= self.n - self.t:
                        ...
                        matching_dealers = [
                            ...
                            if (
                                value['left_commitments'],
                                value['right_commitments'],
                            ) == majority_oc
                        ]
                        excluded_dealers = [
                            d for d, value in outputs.items()
                        ...
                        if fault_controller is not None:
                            fault_controller.record_batchmul_matching_group(
                                matching_dealers=matching_dealers,
                                excluded_dealers=excluded_dealers,
                                left_commitments=majority_oc[0],
                                right_commitments=majority_oc[1],
                            )
                        # Prune all dealers whose (left_commitments, right_commitments) differ
                        for del_d in list(outputs.keys()):
                            val = (outputs[del_d]['left_commitments'], outputs[del_d]['right_commitments'])
                            if val != majority_oc:
                                if fault_controller is not None:
                                    fault_controller.record_batchmul_commitment_exclusion(
                                        ...
                                        left_commitments=outputs[del_d]['left_commitments'],
                                        right_commitments=outputs[del_d]['right_commitments'],
                                    )
                                outputs.pop(del_d, None)
    ...
    async def run_multiply(self, mul_num):
        ...
        fault_controller = getattr(self.mpc_instance, "fault_controller", None)
        if fault_controller is not None:
            fault_controller.record_batchmul_common_subset(acsset)
