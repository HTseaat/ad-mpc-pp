...
class OptimalCommonSet:
    def __init__(
            ...
            acss_outputs, acss_signal, member_list, dealer_ready=None):
        """
    ...
    async def _wait_for_dealers(
        ...
        """Wait until every dealer in ``dealer_ids`` is locally available.

        AggTrans supplies one monotonic Event per dealer.  An Event is set only
        after that dealer has passed ACSS validation and survived commitment
        filtering, and it is never cleared.  This avoids the lost wakeups that
        occur when multiple MVBA tasks share and clear a single Event.

        Other users of OptimalCommonSet still pass only ``acss_signal``.  Keep
        their legacy change-notification path intact for now so this AggTrans
        repair does not alter unrelated protocols.
        """
        ...
        while not all(dealer in outputs for dealer in dealers):
            # Clear and then recheck the predicate before waiting.  This closes
            # the single-waiter check/clear race in the legacy path.
            signal.clear()
    ...
    async def handle_message_dyn(self):
        """
        ...
        def broadcast(o):
            for i in range(self.n):
                logging.info(f"[{self.pid}] otmvba Broadcasting to {self.member_list[i]}")
                optsend(self.member_list[i], o)
    ...
    async def handle_value_message(self, msg, send):
        ...
        send(self.member_list[self.leader], (self.sid, "prevote", self.pid, serialize(sigma1)))
    ...
    async def handle_prevote_qc_message(self, msg, send):
        ...
        send(self.member_list[self.leader], (self.sid, "vote", self.pid, serialize(sigma2)))
    ...
    async def commonsubset(self, rbc_out, acss_outputs, acss_signal, rbc_values, aba_in, aba_out):
        ...
        # Wait only for the ACSS outputs named by the decided common set.
