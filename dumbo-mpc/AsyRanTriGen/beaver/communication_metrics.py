...
class CommunicationMetricsArtifact:
    ...
    def register_batch(self, **entry):
        """Register a public protocol-batch descriptor, deduplicating exact repeats."""
        required = {
            "protocol",
            "tag",
            "source_layer",
    ...
    def write(self, communication, transport, completed, artifact_state="final"):
        ...
        document = {
            ...
            **self.context,
            ...
            "payload_calibrations": sorted(
                self._payload_calibrations,
                key=lambda item: (
                    item["protocol"], item["dealer_local_id"],
                ),
            ),
        }
    ...
    def record_payload_calibration(self, entry):
        """Record public/private dealer payload lengths, never payload contents."""
        if not self.enabled:
            return
        required = {
            "protocol", "dealer_local_id", "public_payload_bytes",
            "private_ciphertext_bytes",
        }
        missing = sorted(required - set(entry))
        if missing:
            raise ValueError(
                "payload calibration is missing: " + ", ".join(missing)
            )
        normalized = dict(entry)
        normalized["dealer_local_id"] = int(normalized["dealer_local_id"])
        normalized["public_payload_bytes"] = int(
            normalized["public_payload_bytes"]
        )
        normalized["private_ciphertext_bytes"] = [
            int(value) for value in normalized["private_ciphertext_bytes"]
        ]
        if normalized["public_payload_bytes"] <= 0 or any(
            value <= 0 for value in normalized["private_ciphertext_bytes"]
        ):
            raise ValueError("payload calibration lengths must be positive")
        self._payload_calibrations.append(normalized)

    def record_proof_component(self, entry):
        """Record one local cryptographic call without serializing its inputs."""
        if not self.enabled:
            return
        required = {
        ...
        normalized = dict(entry)
        for field in ("source_layer", "target_layer", "dealer_local_id", "batch_size", "elapsed_ns"):
        ...
        if normalized["batch_size"] <= 0 or normalized["elapsed_ns"] < 0:
            raise ValueError("proof metric batch_size/elapsed_ns is invalid")
        key = (
    ...
    def record_proof_quorum(self, entry):
        """Freeze the dealer set whose verification counts toward the protocol.

        Raw proof timings continue to contain late/background verification;
        analyzers use this public control-flow marker for the paper's n-t
    ...
    def finalize_proof_operation(self, metadata, required_components, success):
        """Emit a total defined as the sum of named local crypto components."""
        if not self.enabled:
            return
