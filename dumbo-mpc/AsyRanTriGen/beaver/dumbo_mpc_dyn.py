...
class BEAVER:
    ...
    def _should_skip_layer(self, layer_idx):
        """Hook for experiment variants. Default path executes all layers."""
        return False

    def _should_stop_before_layer(self, layer_idx):
        """Hook for permanent crash-stop variants. Default path never stops."""
        return False

    def _on_layer_completed(self, layer_idx):
        """Hook for experiment variants. Default path adds no trace events."""
        return None
    ...
    async def run_beaver(self, node_communicator):
        ...
        for L in range(layers):
            if self._should_stop_before_layer(L):
                logger.warning(
                    f"[{self.my_id}] [layer {L}] process enters permanent "
                    "crash-stop silence; no later layer will execute"
                )
                return
            ...
            self._on_layer_completed(L)

        # === Final reconstruction of last layer outputs ===
        if last_outputs is None:
            raise RuntimeError(
                f"[{self.my_id}] No layer outputs produced before final reconstruction. "
                "Check layer settings or experiment mode."
            )
