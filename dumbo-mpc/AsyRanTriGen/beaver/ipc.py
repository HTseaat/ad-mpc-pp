...
CHANNEL_CONTROL_MARKER = "continuum-curve-channel-control-v1"
CHANNEL_CONTROL_PING = "ping"
CHANNEL_CONTROL_ACK = "ack"


def resolve_auth_mode(auth_mode=None):
    """Resolve and validate the transport authentication mode.

    An explicit argument takes precedence over ``ZMQ_AUTH_MODE``.  Keeping the
    default as ``null`` preserves the transport used by the existing
    experiments until CURVE is wired into ``NodeCommunicator``.
    """
    value = (
        os.environ.get(AUTH_MODE_ENV, AUTH_MODE_NULL)
...
def resolve_io_threads(io_threads=None):
    ...
    Distributed runs preserve the historical one-thread-per-host-CPU default.
    Large single-host experiments can set ``ZMQ_IO_THREADS=1`` so hundreds of
    protocol processes do not each create a full machine-sized I/O pool.
    """
    value = (
...
def validate_curve_config(peers_config, my_id, auth_config):
    """Validate CURVE material and return a socket-ready configuration."""
    if not zmq.has("curve"):
        raise RuntimeError("the installed libzmq/PyZMQ build does not support CURVE")
    if not isinstance(auth_config, dict):
        raise ValueError(
            "ZMQ_AUTH_MODE=curve requires the node's authenticated-channel config"
        )
    ...
    try:
        ...
        raise ValueError(f"{CURVE_ZAP_DOMAIN} is not valid UTF-8") from exc
...
class CurveCredentialsProvider(object):
    """ZAP callback backed by the configured global public-key registry."""
...
class NodeCommunicator(object):
    ...
    def __init__(
        ...
        if self.auth_mode == AUTH_MODE_CURVE:
            self.curve_config = validate_curve_config(
                peers_config, my_id, auth_config
            )
        ...
        self.benchmark_logger.info("ZeroMQ auth mode: %s", self.auth_mode)
    ...
    def write_metrics_checkpoint(self):
        """Persist protocol-complete metrics before any global exit barrier.

        The normal context-manager exit atomically overwrites this checkpoint
        with the drained transport snapshot.  Keeping the same filename means
        collectors see exactly one artifact per logical process, while a
        stalled peer can no longer prevent completed processes from saving
        their local proof timings.
        """
        ...
        try:
            if self.drain_on_exit:
                ...
                for task in self._dealer_tasks:
                    task.cancel()
                await asyncio.gather(*self._dealer_tasks, return_exceptions=True)
                logging.debug("Dealer tasks cancelled.")
        finally:
            ...
            self.communication_metrics.write(
                ...
                {
                    "auth_mode": self.auth_mode,
                    "drained_on_exit": self.drain_on_exit,
                    "channel_setup_ms": self.channel_setup_ms,
                },
                completed=exc_type is None and self.drain_on_exit,
                artifact_state="final",
            )
            if self.auth_mode == AUTH_MODE_CURVE:
                denied = self._curve_credentials.denied_count
                self.benchmark_logger.info(
                    "CURVE authentication failures: %d; invalid metadata: %d; "
                    "identity spoofing events: %d",
                    denied,
                    self.invalid_auth_metadata_count,
                    self.identity_spoofing_count,
                )
    ...
    def _start_curve_authenticator(self):
        self._curve_credentials = CurveCredentialsProvider(
            self.curve_config["zap_domain"],
            self.curve_config["public_keys"],
        )
    ...
    def snapshot_communication_metrics(self):
        """Return a detached, deterministic snapshot of remote payload counters."""
        tags = {}
    ...
    def record_payload_calibration(self, **entry):
        self.communication_metrics.record_payload_calibration(entry)
    ...
    def _handle_channel_control(self, sender_id, msg):
        ...
        if msg[1] != self.curve_config["run_id"]:
            logging.warning(
                "Dropping CURVE readiness message with a mismatched run_id "
                "from authenticated sender %d",
                sender_id,
            )
            return True
    ...
    async def _setup(self):
        ...
        for i in range(len(self.peers_config)):
            if i != self.my_id:
                ...
                # In CURVE mode this routing identity is checked against the party ID
                # authenticated by the ZAP User-Id metadata before payload decoding.
        ...
        if self.auth_mode == AUTH_MODE_CURVE and self.readiness_barrier:
            ...
            self.benchmark_logger.info(
                "CURVE channel ready: peers=%d channel_setup_ms=%.3f",
                len(self._readiness_expected),
                self.channel_setup_ms,
            )
        ...
        while True:
            if self.auth_mode == AUTH_MODE_NULL:
                ...
                if len(frames) != 2:
                    ...
                    logging.warning(
                        "Dropping CURVE message with %d frames; expected 2", len(frames)
                    )
                    continue
                ...
                if authenticated_sender is None:
                    self.invalid_auth_metadata_count += 1
                    logging.warning(
                        "Dropping CURVE message with missing or unmapped User-Id"
                    )
                    continue

                sender_id = bytes(routing_frame)
                expected_routing_id = str(authenticated_sender).encode("ascii")
                if sender_id != expected_routing_id:
...
class ProcessProgramRunner(object):
    def __init__(
        ...
        # Distributed protocol entry points load the per-party CURVE identity
        # alongside their existing cryptographic material in ``extra``.
