...
class PVMultiplicationPayload:
    ...
    @staticmethod
    def from_bytes(data: bytes) -> "PVMultiplicationPayload":
        obj = json.loads(data.decode())
        obj["enc_results"] = [EncResult(**er) for er in obj["enc_results"]]
        return PVMultiplicationPayload(**obj)
...
class PVMultiplicationMsg:
    ACSS = "DT_ACSS"
    ACS  = "DT_ACS"

class PVMultiplication:
    def __init__(self,
                 ...
                 comandproof_right_inputs):                   # list of KZG commitments C_k^l, local secret shares [s_k^l],evaluation proofs [w_k^l]
    ...
    # ------------------------------------------------------------------
    def _prepare_pvmultiplication(self, serialized_left_proof, serialized_right_proof):
        """
        Build the PV‑Multiplication payload (enc_results, commitment, aggregated
        proofs, ...) and store it in `self.pvmultiplication_bytes`.
        ...
        # Keep as str (JSON‐encoded) – DO NOT .encode(), otherwise json.dumps later fails
        ser_aggproofAtZero = json.dumps(dser_aggproofAtZero)
        ...
        pv_obj = PVMultiplicationPayload(
        ...
        logging.info("[PVMULTIPLICATION] PVMultiplicationPayload built successfully")
        self.pvmultiplication_bytes = pv_obj.to_bytes()
    def __enter__(self):
        return self    
    ...
    async def run_pvmultiplication(self, node_communicator):
        start_time = time.time()

        logger.info(f"[{self.my_id}] Starting PVMultiplication...")        
        ...
        tob = YosoTOB(
            ...
            predicate_name="PVmultiplication"
        )
        await tob.start()          # 挂后台
        ...
        try:
            ...
            logging.error("[PVMULTIPLICATION] failed to build sk_hex_map: %s", e)
            raise
        else:
            logging.info("sk_hex_map: %s", self.sk_hex_map)

        # ——业务进程——
        self._prepare_pvmultiplication(serialized_left_proof, serialized_right_proof)
        logging.info("[PVMULTIPLICATION] PVMultiplicationPayload prepared, size= bytes")

        # --- ② 广播 & 等待 Deliver -----------------------------------
        await tob.broadcast(self.pvmultiplication_bytes)
        # —— ③ 等待至少一个区块写入 L_P 并获取内容 ——
        height, block_messages = await tob.wait()            # 返回 (height, List[bytes])
        logging.info(f"[PVMULTIPLICATION] Delivered block height={height}, msg count={len(block_messages)}")

        # —— 额外：也可直接通过 get_last_block() 获取最新区块 —— 
        last_block = tob.get_last_block() or []
        logging.info(f"[PVMULTIPLICATION] get_last_block returned {len(last_block)} messages")
        ...
        for idx, raw_entry in enumerate(last_block):
            try:
                pv_obj = PVMultiplicationPayload.from_bytes(raw_entry)
                ...
                logging.error("[PVMULTIPLICATION] failed to parse block entry %d: %s", idx, e)

        # —— ElGamal 解密 —— 
        for i, entry in enumerate(self.received_entries):
            dealer_id = entry["dealer_id"]
            logging.info("[PVMULTIPLICATION] processing entry %d (dealer %d)", i, dealer_id)
            sk_hex    = self.sk_hex_map.get(dealer_id)
            if sk_hex is None:
                logging.error("[PVMULTIPLICATION] no sk for dealer %d (entry %d)", dealer_id, i)
                continue
            ...
            logging.info("[PVMULTIPLICATION] entry %d (dealer %d) ElGamal decrypted: %s",
            ...
            try:
                ...
                logging.error("[PVMULTIPLICATION] failed to parse g^k point: %s (raw: %s)", e, msg_str)
                gk_hex = None
            entry["gk"] = gk_hex
        ...
        for i, entry in enumerate(self.received_entries):
            ...
            sk_hex = self.sk_hex_map.get(dealer_id)
            if sk_hex is None:
                logging.error("[PVMULTIPLICATION] no sk for dealer %d in symmetric decrypt", dealer_id)
                continue
            gk_hex = entry.get("gk")
            plaintexts = []
            for j, (c_sym, cp_sym) in enumerate(entry.get("cipher_shares", [])):
                ...
                logging.info("[PVMULTIPLICATION] entry %d sym decrypt %d: m=%s, m_prime=%s", i, j, m_hex, mp_hex)
        ...
        logging.info("[PVMULTIPLICATION] common: %s", common)
        ...
        logging.info("[PVMULTIPLICATION] ser_commit: %s", ser_commit.decode("utf-8"))
        logging.info("[PVMULTIPLICATION] ser_share: %s", ser_share.decode("utf-8"))
        # 3. 调用外部 C 接口进行插值
        interpolated_sym = lib.pyInterpolateShareswithTransfer(ser_common, ser_commit, ser_share)
        logging.info("[PVMULTIPLICATION] interpolated symmetric shares: %s", interpolated_sym.decode("utf-8"))

        # --- ③ 优雅关闭 ---------------------------------------------
        await tob.stop()             # 兼容别名 kill()

        logger.info(
            "[%d] PV-Multiplication broadcast done – len=%d bytes",
            self.my_id, len(self.pvmultiplication_bytes)
        )       
        ...
        logger.info(f"[{self.my_id}] [asynchronous pvmultiplication] Finished! Node {self.my_id}, total number: {int ((self.t + 1) * self.batchsize / 2) }, time: {multiplication_time} (seconds)")
