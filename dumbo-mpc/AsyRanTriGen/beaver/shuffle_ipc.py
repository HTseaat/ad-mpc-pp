"""Adjacent-only ZeroMQ transport for the Continuum shuffle runner."""
...
from beaver.ipc import (
    AUTH_MODE_CURVE,
    CurveCredentialsProvider,
    NodeCommunicator,
)
from beaver.utils.misc import print_exception_callback, subscribe_recv, wrap_send
...
def adjacent_peer_ids(global_id, n, physical_layers):
    ...
    if layer > 0:
        permitted_layers.add(layer - 1)
    if layer + 1 < physical_layers:
        permitted_layers.add(layer + 1)

    peers = {
        candidate
        for peer_layer in permitted_layers
...
class AdjacentNodeCommunicator(NodeCommunicator):
    """NodeCommunicator restricted to the shuffle protocol's adjacent graph."""
    ...
    def __init__(self, peers_config, my_id, allowed_peer_ids, *args, **kwargs):
        ...
        invalid = sorted(
            peer
            for peer in allowed
            if not isinstance(peer, int) or not 0 <= peer < len(peers_config)
        )
        if invalid or my_id in allowed:
            raise ValueError(
                "invalid adjacent peer allowlist: "
                f"invalid={invalid}, self={my_id in allowed}"
            )
    ...
    def send(self, node_id, msg):
        if node_id != self.my_id and node_id not in self.allowed_peer_ids:
            raise ValueError(
                "forbidden non-adjacent send: "
                f"source={self.my_id}, destination={node_id}"
            )
    ...
    async def _setup(self):
        ...
        if self.auth_mode == AUTH_MODE_CURVE and self.readiness_barrier:
            ...
            self.benchmark_logger.info(
                "CURVE channel ready: topology=adjacent peers=%d "
                "channel_setup_ms=%.3f",
                len(self.allowed_peer_ids),
    ...
    async def _recv_loop(self, router):
        while True:
            ...
            if sender_id != self.my_id and sender_id not in self.allowed_peer_ids:
                ...
                logging.error(
                    "Dropping forbidden non-adjacent message: "
...
class ShuffleProcessProgramRunner:
    """Runner wrapper that leaves Continuum's generic IPC path unchanged."""
