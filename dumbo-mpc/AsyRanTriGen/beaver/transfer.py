...
def _aggtrans_quorum_protocol(acss, requested_mode):
    """Name the frozen quorum after the ACSS feature switch is resolved."""
    effective_mode = getattr(acss, "mode", requested_mode)
    return (
        "aggtrans-noagg"
        if effective_mode == "avss_with_transfer" else "aggtrans"
    )
...
class Transfer_Foll(DynamicTransfer):
    ...
    async def acss_step(
            self, msgmode, outputs, trans_num, quorum_proposal,
            dealer_ready):
            ...
            if majority_oc is not None:
                ...
                if original_commitments != majority_oc:
                    ...
                    if fault_controller is not None:
                        fault_controller.record_aggtrans_commitment_exclusion(
                            dealer_local_id=dealer,
                            commitments=original_commitments,
                        )
                    continue
                # A late matching dealer is now permanently available to any
                # MVBA task waiting for that specific ACSS output.
                ...
                for oc_val, count in commit_counter.items():
                    if count >= self.n - self.t:
                        majority_oc = oc_val
                        matching_dealers = [
                            d for d, value in outputs.items()
                            if value['original_commits'] == majority_oc
                        ]
                        excluded_dealers = [
                            d for d, value in outputs.items()
                            if value['original_commits'] != majority_oc
                        ...
                        if fault_controller is not None:
                            fault_controller.record_aggtrans_matching_group(
                                matching_dealers=matching_dealers,
                                excluded_dealers=excluded_dealers,
                                commitments=majority_oc,
                            )
                        # Prune all dealers whose original_commitments differ
                        for del_d in list(outputs.keys()):
                            if outputs[del_d]['original_commits'] != majority_oc:
                                if fault_controller is not None:
                                    fault_controller.record_aggtrans_commitment_exclusion(
                                        dealer_local_id=del_d,
                                        commitments=outputs[del_d]['original_commits'],
                                    )
                                outputs.pop(del_d, None)
                        # Publish availability only after commitment pruning.
                        # These per-dealer Events are monotonic and must never
                        # be cleared by MVBA waiters.
                        for accepted_dealer in outputs:
                            dealer_ready[accepted_dealer].set()

                        # Freeze exactly n-t dealers at the quorum boundary.
                        # Late matching outputs remain available to MVBA but
                        # cannot race into this node's initial proposal.
                        ...
                        if callable(record_proof_quorum):
                            record_proof_quorum(
                                protocol=_aggtrans_quorum_protocol(
                                    self.acss, msgmode
                                ),
    ...
    async def run_transfer(self, trans_num):
        ...
        # OptimalCommonSet still accepts the legacy shared signal for its
        # other callers.  AggTrans uses dealer_ready instead.
        legacy_acss_signal = asyncio.Event()
        ...
        fault_controller = getattr(self.mpc_instance, "fault_controller", None)
        if fault_controller is not None:
            fault_controller.record_aggtrans_common_subset(acsset)
