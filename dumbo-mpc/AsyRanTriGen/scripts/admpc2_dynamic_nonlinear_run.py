...
from beaver.admpc2_dynamic_nonlinear import ADMPC_Multi_Layer_Control, ADMPC_Dynamic
...
def _communication_metrics_context(n, t, layers, my_id, my_send_id, total_cm):
    depth = layers - 2
    batch_size = int(total_cm / depth) if depth > 0 else int(total_cm)
    return {
        ...
        "parameters": {
            ...
            "total_cm": int(total_cm), "configured_width": int(batch_size),
            "expected_batch_size": int(batch_size), "expected_operations": int(depth),
        ...
        "selection": {
            "included_tags": [f"M{target}" for target in range(2, layers)],
