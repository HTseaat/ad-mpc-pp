...
def main():
    ...
    parser.add_argument(
        '--ip-file',
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ip.txt'),
        help='每个物理服务器一行 IP；默认使用 scripts/ip.txt',
    )
    parser.add_argument(
        '--output-dir',
        help='可选输出目录；默认按 protocol/total_cm/layers/N 写入 ../conf',
    )
    ...
    # 4) 构造 peers 列表：每层的同一物理服务器使用一个独立端口。
    # 同时为所有 N * layers 个逻辑进程生成一组运行级 CURVE 身份；每份配置
    # 只保存自己的私钥和完整的公钥允许列表。
    ip_file = os.path.abspath(args.ip_file)
    ...
    process_count = N * effective_layers
    ...
    output_dir = (
        ...
        if args.output_dir
            ...
            script_dir, '..', 'conf', f"{protocol}_{total_cm}_{layers}_{N}"
        )
    )
