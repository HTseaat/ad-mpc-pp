...
from beaver.batch_multiplication import BatchMultiplication
from beaver.pvmultiplication import PVMultiplication
import asyncio
import time, sys
import logging
from ctypes import *
...
async def _run(peers, pbk, pvk, n, t, my_id, batchsize, pks, sk, srs, start_time):
    ...
    async with ProcessProgramRunner(peers, n, t, my_id) as runner:
        ...
        # for the current node from init_inputs/node{my_id}_left.json and node{my_id}_right.json
        # -------------------------------------------------------------
        left_path = os.path.join(
            os.path.dirname(__file__), "..", "init_inputs", f"node{my_id}_left.json"
        )
        right_path = os.path.join(
            os.path.dirname(__file__), "..", "init_inputs", f"node{my_id}_right.json"
        )

        if not os.path.exists(left_path):
            left_path = f"init_inputs/node{my_id}_left.json"
        if not os.path.exists(right_path):
            right_path = f"init_inputs/node{my_id}_right.json"

        with open(left_path, "r") as f_left:
            comandproof_left_inputs = json.load(f_left)
        with open(right_path, "r") as f_right:
            comandproof_right_inputs = json.load(f_right)

        # logging.info("run_batch comandproof_left_inputs: %s", comandproof_left_inputs)
        # logging.info("run_batch comandproof_right_inputs: %s", comandproof_right_inputs)

        send, recv = runner.get_send_recv("")
        with PVMultiplication(
            pks,
            sk,
            pbk,
            pvk,
            ...
            recv,
            batchsize,             # B secrets
            comandproof_left_inputs,           # KZG commitments C_k^ℓ, local secret shares s_k^ℓ, evaluation proofs w_k^ℓ
            comandproof_right_inputs,           # KZG commitments C_k^ℓ, local secret shares s_k^ℓ, evaluation proofs w_k^ℓ
        ) as pvmultiplication:
            while True:
                if time.time() > start_time:
                    break
                time.sleep(0.1)
            pvmultiplication_task = asyncio.create_task(pvmultiplication.run_pvmultiplication(runner.node_communicator))
            await pvmultiplication_task
            pvmultiplication.kill()
            pvmultiplication_task.cancel()
