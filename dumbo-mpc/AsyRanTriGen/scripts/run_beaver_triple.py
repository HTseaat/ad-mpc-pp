...
def _resolve_beaver_class(mode):
    ...
    elif normalized in ("fault-accumulation", "fault_accumulation"):
        module_name = "beaver.dumbo_mpc_dyn_fault_accumulation"
    else:
        raise ValueError(
            f"Unknown dumbo mode: {mode}. "
            "Expected one of: full, drop-epoch4, fault-accumulation"
        )
...
async def _run(peers, pbk, pvk, n, t, my_id, batchsize, layers, pks, sk, srs, start_time, beaver_cls):
    ...
    async with ProcessProgramRunner(
        peers, n, t, my_id, auth_config=HbmpcConfig.extras
    ) as runner:
