...
from beaver.gradedgather import GradedGather
import asyncio
import time, sys
import logging
from ctypes import *
...
async def _run(peers, pbk, pvk, n, t, my_id, batchsize, pks, sk, srs, start_time):
    ...
    async with ProcessProgramRunner(peers, n, t, my_id) as runner:
        ...
        with GradedGather(
            ...
            send,
            recv,
            str(batchsize + my_id).encode()
        ) as yosogradedgather:
            while True:
                if time.time() > start_time:
                    break
                time.sleep(0.1)
            yosogradedgather_task = asyncio.create_task(yosogradedgather.run_graded_gather(runner.node_communicator))
            await yosogradedgather_task
            yosogradedgather.kill()
            yosogradedgather_task.cancel()
