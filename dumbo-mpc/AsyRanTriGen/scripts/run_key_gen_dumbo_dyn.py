...
if __name__ == "__main__":
    ...
    parser.add_argument('--ip-file', default=None,
                        help='path to ip.txt (default: scripts/ip.txt next to this file)')
    parser.add_argument('--port', type=int, default=7001,
                        help='peer port (default: 7001)')
    ...
    curve_run_id = uuid.uuid4().hex
    curve_zap_domain = f"dumbo-mpc-{curve_run_id}"

    # 从 ip.txt 读取 peers
    ip_file = args.ip_file or os.path.join(os.path.dirname(os.path.abspath(__file__)), "ip.txt")
    if not os.path.exists(ip_file):
        raise FileNotFoundError(f"未找到 IP 列表文件: {ip_file}")

    with open(ip_file, "r", encoding="utf-8") as fip:
        ips = [line.strip() for line in fip if line.strip() and not line.strip().startswith("#")]

    if len(ips) < N:
        raise ValueError(f"ip.txt 里的 IP 数量({len(ips)}) 少于 N={N}")

    peers = [f"{ip}:{port}" for ip in ips[:N]]
    ...
    for i in range(N):
        ...
        data = {
            ...
            "extra": {
                ...
                "pks_acss": pks_acss[i],
                "sk_acss": sk_acss[i],
                "curve_public_key": curve_public_keys[i],
                "curve_secret_key": curve_keypairs[i][1].decode("ascii"),
