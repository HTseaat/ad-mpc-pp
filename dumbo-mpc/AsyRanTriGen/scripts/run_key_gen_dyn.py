...
if __name__ == "__main__":
    ...
    if election_mode not in ("off", "shadow", "gated"):
        raise ValueError(
            "COMMITTEE_ELECTION_MODE must be one of off, shadow, or gated"
        )
    election_candidate_count = int(
        os.getenv("COMMITTEE_ELECTION_CANDIDATES", "4")
    )
    if election_candidate_count < 1:
        raise ValueError("COMMITTEE_ELECTION_CANDIDATES must be positive")
    if election_mode != "off" and layers < 4:
        raise ValueError("committee-election pipeline requires at least four layers")
    if election_mode != "off" and election_candidate_count > layers - 1:
        raise ValueError(
            "candidate count exceeds the pre-provisioned non-client committees"
        )
    ...
    # CURVE identities belong to global transport endpoints, not to the
    # committee-local node_id.  Thus every (layer, node_id) pair gets an
    # independent key pair indexed by my_send_id = layer * N + node_id.
    curve_keypairs = [zmq.curve_keypair() for _ in range(total_nodes)]
    curve_public_keys = [
        public_key.decode("ascii") for public_key, _ in curve_keypairs
    ]
    curve_zap_domain = "continuum-v1"
    ...
    election_run_id = (
        ...
        else "82d7c0b8040f4ca1b3ff6b9d27888fef"
    )
    election_registry = None
    if election_mode != "off":
        candidates = []
        for candidate_index in range(election_candidate_count):
            physical_layer = candidate_index + 1
            members = []
            for node_id in range(N):
                global_id = physical_layer * N + node_id
                members.append(
                    CommitteeMember(
                        global_party_id=global_id,
                        endpoint_id=peers[global_id],
                        authenticated_identity_digest=hashlib.sha256(
                            curve_public_keys[global_id].encode("ascii")
                        ).hexdigest(),
                        protocol_public_key_digest=hashlib.sha256(
                            acss_pk_layers[physical_layer][node_id].encode("ascii")
                        ).hexdigest(),
                    )
                )
            candidates.append(
        ...
        election_registry = CandidateRegistry(
            registry_id=f"continuum-{election_run_id}",
            committee_size=N,
        ...
        for node_id in range(N):
            ...
            data = {
                ...
                "extra": {
                    ...
                    "curve_public_key": curve_public_keys[my_send_id],
                    "curve_secret_key": curve_keypairs[my_send_id][1].decode("ascii"),
            ...
            if election_mode != "off":
                data["extra"].update(
                    {
                        "committee_election_mode": election_mode,
                        "election_public_keys": tbls_pub_layers,
                        "election_registry": election_registry.to_dict(),
                    }
                )
