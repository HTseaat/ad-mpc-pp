...
pub fn get_seeded_RNG_with_rng<R: RngCore + CryptoRng>(entropy_size: usize, rng: &mut R) -> RAND_impl {
...
pub fn get_seeded_RNG(entropy_size: usize) -> RAND_impl {
    let mut entropy = vec![0; entropy_size];
...
fn get_RAND(entropy_size: usize, entropy: &[u8]) -> RAND_impl {
    // let mut r = RAND::new();
    let mut r = RAND_impl::new();
    r.clean();
