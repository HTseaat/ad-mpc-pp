...
lazy_static! {
    static ref GENERATOR: Mutex<G1> =
        Mutex::new(G1::from_msg_hash(b"default_generator"));
}

pub fn set_generator(g: &G1) {
    *GENERATOR.lock().unwrap() = g.clone();
}
fn generator() -> G1 {
    GENERATOR.lock().unwrap().clone()
}
...
impl Secret {
    /// 用已有的 sk 构造一个 Secret
    pub fn from_scalar(sk: FieldElement) -> Self {
        Secret { sk }
    }
}

impl Public {
    /// 用已有的 pk 点构造一个 Public
    pub fn from_point(pk: G1) -> Self {
        Public { pk }
    }
}
