...
async def rbc_dyn(sid, pid, n, f, leader, input, receive, send, rbc_list, commitee_size):  
    """Reliable broadcast
    :param int pid: ``0 <= pid < N``
    :param int N:  at least 3
    :param int f: fault tolerance, ``N >= 3f + 1``
    ...
    echo_threshold = 2 * f +1  # Wait for ECHO to send READY. (# noqa: E221)
    ready_threshold = f + 1  # Wait for READY to amplify. (# noqa: E221)
    output_threshold = 2 * f + 1  # Wait for this many READY to output
    # NOTE: The above thresholds  are chosen to minimize the size
    # of the erasure coding stripes, i.e. to maximize K.
    ...
    #     for i in range(n):
    #         send(i, o)
    logging.info("rbc_list: %s" % rbc_list)
    logging.info("committee_size: %d" % commitee_size)
    logging.info("n: %d" % n)
    logging.info("f: %d" % f)
    logging.info("pid: %d" % pid)
    logging.info("leader: %d" % leader)

    def broadcast(o):
        for i in range(1, len(rbc_list)):
            logging.info("broadcast Sending to %d: %s" % (i, rbc_list[i]))
            send(rbc_list[i], o)
    ...
    if pid == int(leader%(n)):
        ...
        stripes = encode(k, commitee_size, m)
        mt = merkle_tree(stripes)  # full binary tree
        roothash = mt[1]

        for i in range(commitee_size):
            logging.info(f"[{pid}] Sending to {i}: {rbc_list[i+1]}")    
            branch = get_merkle_branch(i, mt)
            # send(i, (sid, "VAL", roothash, branch, stripes[i]))
            send(rbc_list[i+1], (sid, "VAL", roothash, branch, stripes[i]))
    ...
    from_leader = None
    stripes = defaultdict(lambda: [None for _ in range(commitee_size)])
    echo_counter = defaultdict(lambda: 0)
    echo_senders = set()  # Peers that have sent us ECHO messages
    ready = defaultdict(set)
    ready_sent = False
    ...
    def decode_output(roothash):
        logging.info("Decoding output for roothash: %s" % roothash)
        # Rebuild the merkle tree to guarantee decoding is correct
        m = decode(k, commitee_size, stripes[roothash])
        _stripes = encode(k, commitee_size, m)
        _mt = merkle_tree(_stripes)
        _roothash = _mt[1]
        # TODO: Accountability: If this fails, incriminate leader
        assert _roothash == roothash
    ...
    while True:  # main receive loop
        ...
        if msg[1] == "VAL" and from_leader is None:
            # Validation
            logging.info(f"[{pid}] Received VAL message from {sender}")
            (_, _, roothash, branch, stripe) = msg
            # if sender != leader:
            if n == commitee_size + 1:
                logging.info("entering n == commitee_size + 1")
                logging.info(f"[{pid}] sender: {sender}, leader: {leader}")
            ...
            try:
                if pid < leader: 
                    assert merkle_verify(commitee_size, stripe, roothash, branch, pid)
                else:
                    assert merkle_verify(commitee_size, stripe, roothash, branch, pid - 1)
                logging.info(f"[{pid}] Successfully validated VAL message")
            except Exception as e:
            ...
            # Update
            from_leader = roothash
            logging.info(f"[{pid}] Broadcasted ECHO message for roothash {roothash}")
            broadcast((sid, "ECHO", roothash, branch, stripe))
            

        elif msg[1] == "ECHO":
            logging.info(f"[{pid}] Received ECHO message from {sender}")
            (_, _, roothash, branch, stripe) = msg
            # Validation
            if (
                roothash in stripes
                and stripes[roothash][sender % commitee_size] is not None
                or sender in echo_senders
            ):
            ...
            # Update
            stripes[roothash][sender % commitee_size] = stripe
            echo_senders.add(sender)
            echo_counter[roothash] += 1

            if echo_counter[roothash] >= echo_threshold and not ready_sent:
                ready_sent = True
                logging.info(f"[{pid}] Broadcasted READY message for roothash {roothash}")
                broadcast((sid, "READY", roothash))
            ...
            logging.info(f"[{pid}] Received READY message from {sender}")
            (_, _, roothash) = msg
            # Validation
            if sender in ready[roothash] or sender in ready_senders:
                logger.info("[{pid}] Redundant READY")
