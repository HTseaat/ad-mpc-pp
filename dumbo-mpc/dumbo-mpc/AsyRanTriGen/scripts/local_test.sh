...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <module.py> <num_nodes> <batchsize> [dumbo_mode] [layers]"
    echo "Example: $0 scripts/run_beaver_triple.py 4 300 full 10"
    exit 1
fi
...
python scripts/init_batchsize_ip.py --N ${NUM_NODES} --k ${batchsize} --layers ${layers} --dumbo_mode ${dumbo_mode}

# Auto-clean stale local listeners from previous interrupted runs
./scripts/cleanup_local_test.sh "${NUM_NODES}"
