...
Run distributed Continuum delay and/or Byzantine cases.
...
Options:
  ...
  --delay-ms <ms>            Delay injected at C3 handoff (default: 2000)
  --attack-index <index>     Forked batch element (default: 0)
  --start-delay <sec>        Shared future start offset (default: 30)
  --timeout <sec>            Per-process hard timeout (default: 900)
  --sleep-between-case <sec> Pause between the two cases (default: 15)
  --reuse-config             Reuse an existing local/remote configuration
  --skip-remote-cleanup      Do not remove leftover Continuum containers
USAGE
}
...
BATCHMUL_EPOCH=4
AUTH_MODE=curve
CONTINUUM_CONFIG_GENERATOR=remote-image
...
export CONTINUUM_CONFIG_GENERATOR
...
SESSION_DIR="${RESULTS_ROOT}/${RUN_TAG}_continuum_adversarial_n${N}"
mkdir -p "$SESSION_DIR"

echo "Adversarial session: $SESSION_DIR"
...
cleanup_remote() {
  ...
  if [[ -n "${CLUSTER_ENV:-}" ]]; then
    CLUSTER_ENV="$CLUSTER_ENV" "${SCRIPT_DIR}/cleanup_remote_ports.sh" \
      --protocol continuum --n "$N"
  else
    "${SCRIPT_DIR}/cleanup_remote_ports.sh" --protocol continuum --n "$N"
  fi
}
...
write_source_manifest() {
  local outdir="$1"
  local paths=(
    "${ASY_DIR}/beaver/adversarial_faults.py"
    "${ASY_DIR}/beaver/aggtrans_commitment_fork.py"
    "${ASY_DIR}/beaver/batchmul_input_commitment_fork.py"
    "${ASY_DIR}/beaver/admpc2_dynamic.py"
    "${ASY_DIR}/beaver/transfer.py"
    "${ASY_DIR}/beaver/batch_multiplication.py"
    "${ASY_DIR}/beaver/hbacss.py"
    "${ASY_SCRIPTS_DIR}/control-node.sh"
    "${ASY_SCRIPTS_DIR}/distribute-admpc.sh"
    "${ASY_SCRIPTS_DIR}/distribute-file.sh"
    "${SCRIPT_DIR}/run_continuum_adversarial_dist.sh"
    "${SCRIPT_DIR}/analyze_adversarial_case.py"
...
run_case() {
  local scenario="$1"
  local case_name="continuum-${scenario}"
  ...
  (
    ...
    if [[ "$REUSE_CONFIG" -eq 1 ]]; then
      if [[ ! -d "${ASY_DIR}/conf/${conf_dir}" ]]; then
        echo "Cannot reuse missing Continuum adversarial config: ${ASY_DIR}/conf/${conf_dir}" >&2
        exit 1
      fi
      validate_generated_config "${ASY_DIR}/conf/${conf_dir}"
      echo "Reusing existing Continuum adversarial config ${conf_dir}"
      cd "${ASY_SCRIPTS_DIR}"
      ./distribute-admpc.sh
    else
      "${SCRIPT_DIR}/generate_continuum_config_remote.sh" \
        "$N" "$T" "$LAYERS_TOTAL" "$TOTAL_CM"
      validate_generated_config "${ASY_DIR}/conf/${conf_dir}"
      cd "${ASY_SCRIPTS_DIR}"
      ./distribute-admpc.sh
    ...
    if [[ "$scenario" == "delay" ]]; then
      ...
      export FAULT_TARGET=handoff
      export FAULT_DELTA_MS="$DELAY_MS"
      unset FAULT_BATCHMUL_EPOCH FAULT_ATTACK_INDEX
    else
    ...
    unset FAULT_ACCUMULATION_MODE FAULT_ACCUMULATION_COUNT FAULT_ACCUMULATION_START_EPOCH
    ./control-node.sh "$conf_dir" admpc2 "$(protocol_start_epoch)" "$RUN_TIMEOUT"
  )
  launch_rc=$?
  set -e
  ...
  mkdir -p "$outdir"
  save_metadata \
    "$outdir" continuum "$case_name" "$N" "$T" "$D" \
    "$LAYERS_TOTAL" "$TOTAL_CM"
  {
    echo "scenario=${case_name}"
    echo "fault_mode=$([[ "$scenario" == delay ]] && echo delay || echo byzantine)"
    echo "fault_target=$([[ "$scenario" == delay ]] && echo handoff || echo aggtrans+batchmul)"
    echo "fault_computation_epoch=${COMPUTATION_EPOCH}"
    echo "fault_batchmul_epoch=$([[ "$scenario" == byzantine ]] && echo "$BATCHMUL_EPOCH")"
  ...
  cp -a "${ASY_SCRIPTS_DIR}/logs" "${outdir}/logs" 2>/dev/null || true
  cp -a "${ASY_DIR}/conf/${conf_dir}" "${outdir}/conf" 2>/dev/null || true
  ...
  if [[ "$scenario" == "delay" ]]; then
    ...
    analyzer_args+=(--batchmul-epoch "$BATCHMUL_EPOCH" --attack-index "$ATTACK_INDEX")
  fi

  set +e
  python3 "${SCRIPT_DIR}/analyze_adversarial_case.py" "${analyzer_args[@]}"
  analysis_rc=$?
  set -e
  {
