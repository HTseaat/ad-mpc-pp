...
case "$case_name" in
  admpc-nonlinear) summary_protocol="admpc" ;;
  continuum-batchmul) summary_protocol="continuum_batchmul" ;;
  bgw-aggtrans) summary_protocol="bgw_aggtrans" ;;
  *)
...
cd "$workspace_root"
"$script_dir/run_figure9_one_layer_case.sh" \
  "$case_name" "$n" "$t" "$width" "$case_root"

/opt/venv/continuum/bin/python "$script_dir/summarize_figure9_communication.py" \
